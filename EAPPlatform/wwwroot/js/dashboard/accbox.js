...
$(function () {
    ...
    $("#datepicker").click(function () {
        ...
        layer.open({
            ...
            , type: 2
            , btn: ['OK', 'Cancel']
            , content: '/Statistics/ACCBoxDashboard/weekSelector'
            , area: ['40%', '50%']
            , success: function (layero, index) {
                //向layer页面传值，传值主要代码
                //   var body = layer.getChildFrame('body', index);
...
function GetHistoryAlarmRate() {
    $.ajax({
        ...
        success: function (retdata) {
            ...
            gethistoryRate('monthly-alarm-rate', monthRate, months, 'Monthly', target);
...
function ExportYieldData(start,end) {
    ...
    $.ajax({
        ...
        url: '/Statistics/ACCBoxDashboard/ExportYieldData',
        beforeSend: function () {
            //alert("downloading report!")
        },
        success: function (response, status, request) {
            ...
            var disp = request.getResponseHeader('Content-Disposition');
            if (disp && disp.search('attachment' != -1)) {
                var form = $('<form method="POST" action="' + '/Statistics/ACCBoxDashboard/ExportYieldData' + '">');
                $.each(params, function (k, v) {
                    form.append($('<input type="hidden" name="' + k +
                        '" value="' + v + '">'));
                });
...
function setTarget(target) {
    $.ajax({
        ...
        url: '/Statistics/ACCBoxDashboard/SetUPD',
        success: function (retdata) {
...
function setAlarmTarget(target) {
    $.ajax({
        type: 'post',
        dataType: 'json',
        data: {
        ...
        url: '/Statistics/ACCBoxDashboard/SetAlarmRateTarget',
        success: function (retdata) {
...
function GetRealTimeAlarm() {
    $.ajax({
        ...
        url: '/Statistics/ACCBoxDashboard/GetRealTimeAlarms',
        success: function (retdata) {
            ...
            if (alarmdata) {
                layui.table.render({
                    ...
                    , cols: [[ //标题栏
                        { field: 'EQID', title: 'EQID', align: "center", width: '20%' }
                        , { field: 'ALARMTIME', title: 'Alarm Time', align: "center", width: '25%' }
                        , { field: 'ALARMTEXT', title: 'Alarm Text', align: "center" }
            ...
            if ($('.nav-overall').hasClass('active')) {
                layui.table.render({
                    ...
                    , cols: [[ //标题栏
                        ...
                        , { field: 'output', title: 'Output', align: "center" }
                        , { field: 'yield', title: 'Yield', align: "center" }
...
function getEQProfie(EQID) {
        ...
        $.ajax({
            ...
            url: '/Statistics/ACCBoxDashboard/GetStationProfile',
            success: function (retdata) {
...
function getAlarmPosition() {
    ...
    return new Promise(function (resolve, reject) {
        $.ajax({
            ...
            url: '/Statistics/ACCBoxDashboard/GetStationAlarms',
            success: function (retdata) {
                //console.log(retdata)
                ...
                const result = alarmPositions.filter(item1 => {
                    ...
                    var obj = matchItem ? { text: item1.alarmtime + '<br>' + item1.alarmtext, position: matchItem.position } : null;
